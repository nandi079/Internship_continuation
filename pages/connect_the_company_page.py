...
class ConnectTheCompanyPage(Page):
    ...
    YOUTUBE_LINK = (By.XPATH, "//div[@class='text-social' and text()= 'Youtube']")
    ...
    def verify_social_media_icons(self):
        ...
        self.wait_until_visible(*self.TELEGRAM_LINK)
        self.verify_text('Telegram', *self.TELEGRAM_LINK)
        self.wait_until_visible(*self.LINKEDIN_LINK)
        self.verify_text('LinkedIn', *self.LINKEDIN_LINK)
        self.wait_until_visible(*self.YOUTUBE_LINK)
        self.verify_text('Youtube', *self.YOUTUBE_LINK)

    def verify_connect_the_company_button_available_and_clickable(self):
